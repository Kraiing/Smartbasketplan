...
export const useBallLogic = (initialPlayers, initialBall, setPlayers) => {
  ...
  const [ballAnimation, setBallAnimation] = useState({
    ...
    startTime: 0,
    duration: 0,
    isHoopShot: false // เพิ่มตัวแปรเพื่อระบุว่าเป็นการยิงไปที่แป้นหรือไม่
  });
  ...
  const calculateArcY = (startY, endY, progress, isHoopShot = false) => {
    ...
    // เพิ่มความสูงของโค้งหากเป็นการยิงลงห่วง
    const arcHeight = isHoopShot 
      ? (isIOS.current ? 12 : 16) // โค้งสูงขึ้นสำหรับการยิงลงห่วง
      : (isIOS.current ? 4 : 8);  // โค้งปกติสำหรับการส่งบอล
  ...
  useEffect(() => {
    ...
    const runAnimation = () => {
      ...
      const isHoopShot = ballAnimation.isHoopShot;
      ...
      const step = () => {
        ...
        const currentY = calculateArcY(startY, endY, easeProgress, isHoopShot);
        ...
        setBall(ballState => ({
          ...
          x: currentX,
          y: currentY,
          // กำหนด holderId เมื่ออนิเมชันเสร็จสิ้น และไม่ใช่การยิงลงห่วง
          holderId: (currentProgress >= 1 && !isHoopShot) ? targetId : null
        }));
        ...
        if (currentProgress < 1 && isMounted) {
          ...
          if (isHoopShot) {
            // ถ้าเป็นการยิงลงห่วง ไม่ต้องกำหนดให้ผู้เล่นถือบอล
            // แต่ให้ลูกบอลหยุดอยู่ที่แป้น (หรืออาจให้ตกลงพื้นตามโค้ดเพิ่มเติม)
            console.log("Ball shot into the hoop!");
            
            // อาจเพิ่มโค้ดให้ลูกบอลตกลงพื้นที่หลังแป้นบาสได้ในอนาคต
          } else if (targetId) {
            // อัพเดตว่าผู้เล่นเป้าหมายถือลูกบอล (กรณีส่งบอลให้ผู้เล่น)
            setPlayers(players =>
              players.map(p => ({
                ...p,
      ...
      safetyTimeoutRef.current = setTimeout(() => {
        if (isMounted) {
          ...
          setBall({
            ...
            holderId: ballAnimation.isHoopShot ? null : ballAnimation.targetPlayerId
          });

          if (!ballAnimation.isHoopShot && ballAnimation.targetPlayerId) {
            setPlayers(players =>
              players.map(p => ({
                ...p,
    ...
    return () => {
      ...
      ballAnimation.endX, ballAnimation.endY, ballAnimation.targetPlayerId, ballAnimation.isHoopShot, setPlayers]);
  ...
  const endBallPass = useCallback((pointerX, pointerY, players) => {
    ...
    try {
      ...
      // เช็คว่าอยู่ใกล้แป้นบาสหรือไม่
      const isNearLeftHoop = Math.abs(pointerX - 10) < 10 && Math.abs(pointerY - 50) < 10;
      const isNearRightHoop = Math.abs(pointerX - 86) < 10 && Math.abs(pointerY - 50) < 10;
      const isNearHoop = isNearLeftHoop || isNearRightHoop;
      
      // บันทึกข้อมูลตำแหน่งแป้นที่ใกล้ที่สุด
      let hoopPosition = null;
      if (isNearLeftHoop) {
        hoopPosition = { x: 10, y: 50 };
      } else if (isNearRightHoop) {
        hoopPosition = { x: 86, y: 50 };
      }
      ...
      if (isNearHoop) {
        // ถ้าส่งบอลไปที่แป้นบาส
        console.log("Ball passed to hoop:", hoopPosition);
        ...
        const distance = Math.sqrt(
          Math.pow(hoopPosition.x - fromX, 2) +
          Math.pow(hoopPosition.y - fromY, 2)
        );
        
        // ใช้ระยะเวลาที่สั้นลงสำหรับอนิเมชันลงแป้น
        const minDuration = isIOS.current ? 700 : 500;
        const maxDuration = isIOS.current ? 1200 : 1000;
        const baseDuration = Math.min(maxDuration, Math.max(minDuration, distance * 15));
        
        console.log("Starting ball animation to hoop with duration:", baseDuration);
        
        // เริ่มอนิเมชันการส่งลูกบอลไปที่แป้น
        setBallAnimation({
          active: true,
          startX: fromX,
          startY: fromY + 2,
          ...
          endY: hoopPosition.y,
          progress: 0,
          targetPlayerId: null, // ไม่มีผู้เล่นเป้าหมาย เพราะเป็นการยิงลงห่วง
          startTime: Date.now(),
          duration: baseDuration,
          isHoopShot: true // ระบุว่าเป็นการยิงลงห่วง
        });
        
      } else if (targetPlayer && targetPlayer.id !== ballOriginPlayerRef.current.id) {
        ...
        setBallAnimation({
          ...
          isHoopShot: false // ไม่ได้เป็นการยิงลงห่วง
        });
